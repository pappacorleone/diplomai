// Basic server with avatar display (no HeyGen API calls)
...
const app = express();
const PORT = 3004; // Use a different port
...
// Mock HeyGen token for UI testing
app.get('/api/heygen/token', async (req, res) => {
  console.log('Mock HeyGen token requested');
  // Return a fake token for UI testing
  res.json({ token: 'mock-token-for-testing-123456789' });
});
...
// Main page route
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, '../minimal-test/trump-avatar.html'));
});
