...
async function createHeyGenSession() {
  try {
    ...
    const response = await new Promise((resolve, reject) => {
      const req = https.request('https://api.heygen.com/v1/streaming.create_token', options, (res) => {
        let data = '';
        res.on('data', (chunk) => {
          data += chunk;
        });
...
app.get('/api/heygen/token', async (req, res) => {
  try {
    const token = await createHeyGenSession();
    if (!token) {
      return res.status(500).json({ error: 'Failed to create HeyGen token' });
    }
    res.json({ token });
  } catch (error) {
